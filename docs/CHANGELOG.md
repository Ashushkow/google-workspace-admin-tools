...
## [2.1.0] - 2025-07-31 - "Масштабное развитие функционала"

### ✨ Крупные новые функции
- **NEW**: Управление организационными единицами (OU) - создание пользователей с выбором OU
- **NEW**: Система управления документами с копированием/вставкой
- **NEW**: Полноценное управление календарями Google Calendar API
- **NEW**: Система тем (светлая, тёмная, синяя) с автосохранением
- **NEW**: Система горячих клавиш с HotkeyManager
- **NEW**: OAuth 2.0 приоритетная авторизация

### 🏗️ Архитектурные революции
- **MAJOR**: Модульная архитектура - 91 Python модуля в src/
- **MAJOR**: Профессиональная структура проекта (api/, ui/, core/, config/, utils/)
- **MAJOR**: Асинхронная загрузка данных без блокировки UI
- **MAJOR**: Принципы SOLID в каждом компоненте

### 🐛 Критические исправления
- **FIX**: ИСПРАВЛЕНА проблема удаления участников из групп Google
- **FIX**: Убраны дублированные методы remove_member
- **FIX**: Заменены заглушки на реальную реализацию Google API
- **FIX**: Все относительные импорты для новой структуры папок

### 📈 Впечатляющая статистика
- **STAT**: Python модулей: ~30 → 91 (+203%)
- **STAT**: Основных функций: 8 → 21+ (+162%)
- **STAT**: API методов: ~50 → 120+ (+140%)
- **STAT**: UI компонентов: 5 → 15+ (+200%)
